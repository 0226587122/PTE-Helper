...
def score_summarize_written_text(text: str | None, key_points: list[str]) -> ScoreResult:
    ...
    if not (5 <= count <= 75) or not one_sentence:
        ...
        return ScoreResult(score=0, max_score=7, detail=detail, zeroed_reason=reason)
    ...
    detail.update(traits=traits, key_points_covered=hits)
    return ScoreResult(score=sum(traits.values()), max_score=7, detail=detail)
...
def score_write_essay(text: str | None, key_points: list[str]) -> ScoreResult:
    ...
    if form == 0:
        detail["traits"] = {"form": 0}
        return ScoreResult(
            score=0, max_score=13, detail=detail,
    ...
    detail.update(traits=traits, key_points_covered=hits, paragraphs=len(paragraphs), linking_words=linking)
    return ScoreResult(score=sum(traits.values()), max_score=13, detail=detail)
...
def score_summarize_spoken_text(text: str | None, key_points: list[str]) -> ScoreResult:
    ...
    if form == 0:
        detail["traits"] = {"form": 0}
        return ScoreResult(
            score=0, max_score=8, detail=detail,
    ...
    detail.update(traits=traits, key_points_covered=hits)
    return ScoreResult(score=sum(traits.values()), max_score=8, detail=detail)
